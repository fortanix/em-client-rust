...
use em_client::{Api, Client, models};
...
lazy_static! {
    static ref LOGIN_FILE: String = format!("{}/.em-login.json", env::var("HOME").expect("Failed getting HOME environment variable"));
...
fn store_login_data(url_str: &str, token_str: &str, root_ca_str: Option<String>) -> Result<(), String> {
    let mut file = File::create(&*LOGIN_FILE).map_err(|e| format!("Failed storing login token in {}, {}", &*LOGIN_FILE, e))?;
    let data = LoginData{ url: url_str.to_string(), token: token_str.to_string(), root_ca_str: root_ca_str };
...
fn get_login_data() -> Result<LoginData, String> {
    let mut f = File::open(&*LOGIN_FILE).map_err(|e| format!("Failed opening login-token file {}, {}", &*LOGIN_FILE, e))?;

    let mut data = String::new();
    f.read_to_string(&mut data).map_err(|e| format!("Failed reading login-token file {}, {}", &*LOGIN_FILE, e))?;
...
fn get_client(url: &str, token: Option<String>, root_ca_str: &Option<String>) -> Client {
    ...
    if let Some(token) = token {
        client.headers().set(Authorization(Bearer {
            token: token,
        }));
    }
    client
...
fn login(client:&mut Client, username: &str, password: &str) -> Result<String, String> {
    ...
    let auth = client.authenticate_user().map_err(|e| format!("Authenticate user call failed, {}", e.0))?;
...
fn parse_sigstruct(path: &str) -> Result<models::CreateBuildRequest, String> {
    ...
    let sigstruct = Sigstruct::try_copy_from(&std::fs::read(path).map_err(|e| format!("{}", e))?).unwrap();
    let mrenclave = to_hex(&sigstruct.enclavehash);

    let mut hasher = Hasher::new(Algorithm::SHA256);
    hasher.write_all(&sigstruct.modulus).map_err(|e| format!("Failed calculating mrsigner: {}", e))?;
    ...
    clap::App::new("em-build-cli")
        ...
        .subcommand(SubCommand::with_name("user")
                    .subcommand(SubCommand::with_name("login")
                                .arg(Arg::with_name("url").required(true).help("URL to login to."))
                                .arg(Arg::with_name("username").required(true).help("Username for authentication if using Basic authentication"))
                                .arg(Arg::with_name("password").required(true).help("Password for authentication if using Basic authentication"))
                                .arg(Arg::with_name("add-root-ca").takes_value(true).long("--add-root-ca").help("Root CA certificate to add for verification")))
        ...
        .subcommand(SubCommand::with_name("account")
                    ...
                    .subcommand(SubCommand::with_name("create")
                                .arg(Arg::with_name("name").required(true).help("Name of the account. Accounts must be unique within an EM instance")))
                    .subcommand(SubCommand::with_name("select")
                                .arg(Arg::with_name("account-id").required(true).help("Account UUID to select"))))
        .subcommand(SubCommand::with_name("app")
                    .subcommand(SubCommand::with_name("list"))
                    .subcommand(SubCommand::with_name("create")
                                .arg(Arg::with_name("name").required(true).help("Name of the app"))
                                .arg(Arg::with_name("isvprodid").required(true).help("ISVPRODID"))
                                .arg(Arg::with_name("isvsvn").required(true).help("ISVSVN"))
                                .arg(Arg::with_name("allowed-domains").help("Comma separated allowed domains")))
                    .subcommand(SubCommand::with_name("update")
                                .arg(Arg::with_name("app-id").required(true).help("App uuid"))
                                .arg(Arg::with_name("allowed-domains").takes_value(true).long("--allowed-domains").help("Comma separated allowed domains"))))
        .subcommand(SubCommand::with_name("build")
                    .subcommand(SubCommand::with_name("list"))
                    .subcommand(SubCommand::with_name("create")
                                .arg(Arg::with_name("app-id").required(true).help("App ID"))
                                .arg(Arg::with_name("sigstruct-path").required(true).help("path to sigstruct to use for build")))
                    .subcommand(SubCommand::with_name("delete")
                                .arg(Arg::with_name("build-id").required(true).help("build uuid")))
                    .subcommand(SubCommand::with_name("parse-sigstruct")
                                .arg(Arg::with_name("path").required(true).help("Path to sigstruct file"))))
        .subcommand(SubCommand::with_name("task")
                    ...
                    .subcommand(SubCommand::with_name("update")
                                .arg(Arg::with_name("task-id").required(true).help("task uuid"))
                                .arg(Arg::with_name("status").required(true).help("approved or denied"))))
        .subcommand(SubCommand::with_name("zone")
                    .subcommand(SubCommand::with_name("list"))
                    .subcommand(SubCommand::with_name("get")
                                .arg(Arg::with_name("zone-id").required(true).help("zone uuid")))
                    .subcommand(SubCommand::with_name("get-join-token")
                                .arg(Arg::with_name("zone-id").required(true).help("zone uuid"))))
    ...
    match matches.subcommand() {
        ("user", Some(matches)) => {
            match matches.subcommand() {
                ("create", Some(param)) => {
                    ...
                    let root_ca_str : Option<String> = param.value_of("add-root-ca").map(|path| std::fs::read_to_string(path).expect("Failed reading root-ca certificate"));
                    ...
                    let result = client.create_user(request).map_err(|e| format!("create-user failed: {}", e))?;
        ...
        ("account", Some(matches)) => {
            match matches.subcommand() {
                ("list", Some(_)) => {
                    ...
                    let result = client.get_accounts().map_err(|e| format!("get-accounts failed: {}", e))?;
                    println!("{}", serde_json::to_string_pretty(&result).unwrap());
                }
                ("create", Some(param)) => {
                    let client = get_cached_client()?;
                    ...
                    let result = client.create_account(request).map_err(|e| format!("create-account failed: {}", e))?;
                    println!("{}", serde_json::to_string_pretty(&result).unwrap());
                }
                ("select", Some(param)) => {
                    let account_id = param.value_of("account-id").unwrap();
                    let account_uuid = Uuid::parse_str(account_id).map_err(|e| format!("select-account UUID parsing failed for \"{}\": {}", account_id, e))?;
                    
                    let client = get_cached_client()?;
                    client.select_account(account_uuid).map_err(|e| format!("select-account failed: {}", e))?;
        ...
        ("app", Some(matches)) => {
            match matches.subcommand() {
                ("list", Some(_)) => {
                    let client = get_cached_client()?;
                    let result = client.get_all_apps(None, None, None, None, None, None).map_err(|e| format!("get_all_apps failed: {}", e))?;
                    println!("{}", serde_json::to_string_pretty(&result).unwrap());
                }
                ("update", Some(param)) => {
                    let client = get_cached_client()?;
                    ...
                    let app_uuid = Uuid::parse_str(app_id).map_err(|e| format!("update-app UUID parsing failed for \"{}\": {}", app_id, e))?;

                    let domains = Some(param.value_of("allowed-domains").unwrap().split(",").map(|s| s.to_string()).collect());
                    ...
                    let result = client.update_app(app_uuid, request).map_err(|e| format!("create-account failed: {}", e))?;
                    println!("{}", serde_json::to_string_pretty(&result).unwrap());
                }
                ("create", Some(param)) => {
                    let client = get_cached_client()?;
                    ...
                    let result = client.add_application(request).map_err(|e| format!("add-application failed: {}", e))?;
                    println!("{}", serde_json::to_string_pretty(&result).unwrap());
                }
                _ => (),
            }
        ...
        ("build", Some(matches)) => {
            match matches.subcommand() {
                ("list", Some(_)) => {
                    let client = get_cached_client()?;
                    let result = client.get_all_builds(None, None, None, None, None, None, None).map_err(|e| format!("get-all-builds failed: {}", e))?;
                    println!("{}", serde_json::to_string_pretty(&result).unwrap());
                }
                ...
                ("delete", Some(param)) => {
                    ...
                    let build_uuid = Uuid::parse_str(build_id).map_err(|e| format!("delete-build UUID parsing failed for \"{}\": {}", build_id, e))?;
                    
                    let client = get_cached_client()?;
                    client.delete_build(build_uuid).map_err(|e| format!("delete-build failed: {}", e))?;
                ...
                ("create", Some(param)) => {
                    ...
                    if let Some(app_id) = param.value_of("app-id") {
                        let app_uuid = Uuid::parse_str(app_id).map_err(|e| format!("create-build UUID parsing failed for \"{}\": {}", app_id, e))?;
                        request.app_id = Some(app_uuid);
                    }
                    
                    let result = client.create_build(request).map_err(|e| format!("create-build failed: {}", e))?;
                    println!("{}", serde_json::to_string_pretty(&result).unwrap());
                }
                _ => (),
            }
        ...
        ("task", Some(matches)) => {
            match matches.subcommand() {
                ("list", Some(_)) => {
                    let client = get_cached_client()?;
                    let result = client.get_all_tasks(None, None, None, None, None, None, None, None, None).map_err(|e| format!("get-all-tasks failed: {}", e))?;
                    println!("{}", serde_json::to_string_pretty(&result).unwrap());
                }
                ("get", Some(param)) => {
                    let task_id = param.value_of("task-id").unwrap();
                    let task_uuid = Uuid::parse_str(task_id).map_err(|e| format!("get-task UUID parsing failed for \"{}\": {}", task_id, e))?;
                    
                    let client = get_cached_client()?;
                    let result = client.get_task(task_uuid).map_err(|e| format!("get-task failed: {}", e))?;
                    println!("{}", serde_json::to_string_pretty(&result).unwrap());
                }
                ("update", Some(param)) => {
                    let task_id = param.value_of("task-id").unwrap();
                    let task_uuid = Uuid::parse_str(task_id).map_err(|e| format!("update-task UUID parsing failed for \"{}\": {}", task_id, e))?;
                    
                    let status = param.value_of("status").unwrap().to_string();
                    let approval_status = models::ApprovalStatus::from_str(status.to_uppercase().as_str()).map_err(|_| format!("expected approved or denied as parameter, got: {}", status))?;
                    let client = get_cached_client()?;
                    ...
                    let result = client.update_task(task_uuid, request).map_err(|e| format!("update-task failed: {}", e))?;
                    println!("{}", serde_json::to_string_pretty(&result).unwrap());
                }
                _ => (),
            }
        ...
        ("zone", Some(matches)) => {
            match matches.subcommand() {
                ("list", Some(_)) => {
                    let client = get_cached_client()?;
                    let result = client.get_zones().map_err(|e| format!("get-zones failed: {}", e))?;
                    println!("{}", serde_json::to_string_pretty(&result).unwrap());
                }
                ("get", Some(param)) => {
                    let zone_id = param.value_of("zone-id").unwrap();
                    let zone_uuid = Uuid::parse_str(zone_id).map_err(|e| format!("get-zone UUID parsing failed for \"{}\": {}", zone_id, e))?;
                    
                    let client = get_cached_client()?;
                    let result = client.get_zone(zone_uuid).map_err(|e| format!("get-zone failed: {}", e))?;
                    println!("{}", serde_json::to_string_pretty(&result).unwrap());
                }
                ("get-join-token", Some(param)) => {
                    let zone_id = param.value_of("zone-id").unwrap();
                    let zone_uuid = Uuid::parse_str(zone_id).map_err(|e| format!("get-zone-join-token UUID parsing failed for \"{}\": {}", zone_id, e))?;
                    
                    let client = get_cached_client()?;
                    let result = client.get_zone_join_token(zone_uuid).map_err(|e| format!("get-zone-join-token failed: {}", e))?;
                    println!("{}", serde_json::to_string_pretty(&result).unwrap());
                }
                _ => (),
            }
        ...
        ("node", Some(matches)) => {
            match matches.subcommand() {
                ("list", Some(_)) => {
                    let client = get_cached_client()?;
                    let result = client.get_all_nodes(None, None, None, None, None, None, None, None).map_err(|e| format!("get_all_nodes failed: {}", e))?;
                    println!("{}", serde_json::to_string_pretty(&result).unwrap());
                }
                _ => (),
            }
